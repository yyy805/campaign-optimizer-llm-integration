...
from campaign_optimizer.llm.reviewer_binding_v13 import ReviewerBindingFailure,next_action_v13
...
def classify(exc):
 ...
 if isinstance(exc,ReviewerBindingFailure):return "binding",f"REVIEWER_BINDING.{exc.code.value}",False
 if isinstance(exc,(ContractValidationError,ValidationError,ValueError,KeyError,TypeError)):return "binding","REVIEWER_BINDING.contract",False
 return "internal","REVIEWER_INTERNAL",False
def run_pilot(adapter,config,cases=CASES):
 ...
 for index,(case_id,expected) in enumerate(cases):
  ...
  while attempts<2:
   ...
   try:
    decision,audit=adapter.call_json(role="reviewer",payload=payload);latency+=audit.latency_ms or 0;tokens+=audit.total_tokens or 0;next_action_v13(decision,packet=packet,revision_rounds=0,max_revision_rounds=0);outcome=decision["decision"];category=code=None;break
 ...
 aggregate={"provider_calls":adapter.ledger.used,"total_latency_ms":round(sum(x["latency_ms"] for x in rows),3),"total_tokens":sum(x["total_tokens"] for x in rows),"matched":sum(x["match"] for x in rows),"case_count":len(rows)};accepted=aggregate["matched"]==len(rows) and not any(x["failure_category"] for x in rows)
 return {"status":"PASS" if accepted else "FAIL","acceptance":{"all_expected_decisions_match":aggregate["matched"]==len(rows),"no_structure_or_safety_failure":not any(x["failure_category"] for x in rows),"accepted":accepted},"aggregate":aggregate,"cases":rows,"scope_note":"Compatibility pilot only; it does not prove overall Reviewer quality."}
def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument("--real",action="store_true");p.add_argument("--case",choices=[x[0] for x in CASES]);a=p.parse_args();config=load_role_configuration();cases=tuple(x for x in CASES if a.case is None or x[0]==a.case)
 if not a.real:print(json.dumps({"status":"DRY_RUN","case_count":len(cases),"selected_case":a.case,"distribution":{k:sum(1 for _,v in cases if v==k) for k in ("PASS","REVISE","REJECT")},"reviewer_call_limit":2*len(cases),"model":config.roles.model_aliases["reviewer"],"tool":config.tool_name,"scope_note":"Compatibility pilot only; it does not prove overall Reviewer quality."},sort_keys=True));return 0
 adapter=RoleCallAdapterV13(config);adapter.set_total_limit(2*len(cases));result=run_pilot(adapter,config,cases);print(json.dumps(result,sort_keys=True));return 0 if result["acceptance"]["accepted"] else 1
