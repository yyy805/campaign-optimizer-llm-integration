...
from campaign_optimizer.llm.three_role_runner_v12 import ReviewerChannelFailure,RoleCallAdapterV12,_safe_structure_reason
...
def classify(exc):
 ...
 if isinstance(exc,RoleCallFailure):
  code=exc.audit.error_code or "PROVIDER";return ("protocol" if code.startswith("REVIEWER_PROTOCOL") else "provider"),code,False
...
def run_pilot(adapter,config):
 ...
 for index,(case_id,expected) in enumerate(CASES):
  ...
  attempts=0;outcome="FALLBACK";category=code=None;latency=0.0;tokens=0
  while attempts<2:
  ...
  match=outcome==expected and code is None;rows.append({"case_id":case_id,"expected":expected,"outcome":outcome,"match":match,"attempts":attempts,"failure_category":category,"safe_code":code,"latency_ms":round(latency,3),"total_tokens":tokens})
...
def main():
 ...
 adapter=RoleCallAdapterV12(config);adapter.set_total_limit(10);result=run_pilot(adapter,config);print(json.dumps(result,sort_keys=True));return 0 if result["acceptance"]["accepted"] else 1
