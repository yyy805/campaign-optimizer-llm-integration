...
CONFIG_V14=ROOT/"campaign_optimizer"/"llm"/"agent_roles.v14.json"
...
def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument("--real",action="store_true");p.add_argument("--case",choices=[case[0] for case in load_cases()]);a=p.parse_args();config=load_role_configuration(CONFIG_V14);cases=tuple(case for case in load_cases() if a.case is None or case[0]==a.case)
