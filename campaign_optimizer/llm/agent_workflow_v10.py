"""v10 configuration loader with pinned Reviewer v4 output discipline."""
...
CONFIG = Path(__file__).with_name("agent_roles.v10.json")
...
def load_role_configuration(path: Path = CONFIG) -> RoleConfigurationV9:
    ...
    if raw.get("schema_version") != "1.0" or raw.get("configuration_version") != "agent_roles_v10":
    ...
    if not isinstance(note, Mapping) or note.get("temporary_general_model_studio_mapping") is not True or note.get("reviewer_revert_model") != REVIEWER_REVERT_MODEL:
        raise ValueError("v10 must remain explicitly temporary and reversible")
