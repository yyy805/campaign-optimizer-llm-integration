"""Temporary general-Model-Studio role mapping over unchanged v8 contracts."""
...
CONFIG = Path(__file__).with_name("agent_roles.v9.json")
TEMPORARY_MODELS = {
    "triage": "qwen3.6-flash",
    "executor": "qwen3.7-max",
    "reviewer": "qwen3.7-plus",
}
REVIEWER_REVERT_MODEL = "qwen3.8-max-preview"


@dataclass(frozen=True)
class RoleConfigurationV9:
    roles: RoleConfiguration
    executor_max_output_tokens: int
    reviewer_revert_model: str
...
def load_role_configuration(path: Path = CONFIG) -> RoleConfigurationV9:
    ...
    if raw.get("schema_version") != "1.0" or raw.get("configuration_version") != "agent_roles_v9":
    ...
    if not isinstance(note, Mapping) or note.get("temporary_general_model_studio_mapping") is not True or note.get("reviewer_revert_model") != REVIEWER_REVERT_MODEL:
        raise ValueError("v9 must remain explicitly temporary and reversible")
    aliases = raw.get("model_aliases")
    artifacts = raw.get("prompt_artifacts")
    expected_hashes = raw.get("expected_prompt_hashes")
    if aliases != TEMPORARY_MODELS:
        raise ValueError("temporary model mapping changed without a new configuration version")
    ...
    if len(set(aliases.values())) != len(ROLES):
        raise ValueError("model aliases must remain heterogeneous")
    ...
    return RoleConfigurationV9(roles, executor_limit, REVIEWER_REVERT_MODEL)
